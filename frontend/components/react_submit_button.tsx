 ...
 * @custom:security Prevents duplicate blockchain transactions on rapid clicks.
 *                  Success state is also blocked to prevent re-submission.
 */
 ...
 * @notice  Returns true when aria-busy should be set (active submission).
 * @param   state                Resolved button state.
 * @param   isLocallySubmitting  True while an async onClick is in-flight.
 */
...
const ReactSubmitButton = ({
  ...
  const blocked = isSubmitButtonInteractionBlocked(
    resolvedState,
    disabled,
    isLocallySubmitting,
  );
